...
def check_zoning(lat: float, lng: float, radius_m: float = 200) -> dict:
    ...
    try:
        ...
        for el in elements:
            tags = el.get("tags", {})
            zone_key = tags.get("landuse") or tags.get("natural") or tags.get("leisure")
            if zone_key and zone_key in ZONE_RULES:
                rule = ZONE_RULES[zone_key]
                return {
                    "zone_type":    zone_key,
...
def check_buildings_on_land(lat: float, lng: float, radius_m: float = 100) -> dict:
    ...
    (
      ...
      way["landuse"="construction"](around:{radius_m},{lat},{lng});
    ...
    try:
        ...
        for el in elements:
            ...
            label = BUILDING_TYPE_LABELS.get(raw_type, raw_type.replace("_", " ").title())
            name  = tags.get("name", "")
            buildings.append({
                "type":  raw_type,
                "label": label,
